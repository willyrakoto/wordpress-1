...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	file_env 'WORDPRESS_DB_HOST' "${MYSQL_SERVICE_HOST:-mysql}"
	# if we're linked to MySQL and thus have credentials already, let's use them
	file_env 'WORDPRESS_DB_USER' "${MYSQL_USER:-}"
